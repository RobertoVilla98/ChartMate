...
- **Fix Risolutivo del Rendering Dati (Plotly Data Engine & Timeframe)**:
  ...
  - **Isolamento CSS delle Schede (`.view-panel`)**: Risolto il conflitto di specificità CSS dove una precedente direttiva `display: none !important;` oscurava i pannelli anche in presenza della classe `.active`. Rimosso `!important` a favore della specificità standard BEM/CSS (`.view-panel { display: none; }` e `.view-panel.active { display: flex; }`), garantendo la corretta visualizzazione di tutte e tre le schede e inserito il placeholder dedicato per la scheda `3. MultiPlot Grid`.
